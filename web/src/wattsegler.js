...
import data from "./wattsegler.json";
    ...
    .then((html) => {
      ...
      $html.find("table tr").each((i, tr) => {
        ...
        let m = L.circleMarker(loc, {
          ...
          .bindPopup(
            `<div class="wattsegler"><div class="name">${name}</div>${tab}<div class="source">source <a href="${data.url}" target="_blank">wattsegler.de</a></div></div>`,
          )
          .addTo(layer);
      });
    })
