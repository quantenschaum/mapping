...
export function init_ais(map, wsurl, layer) {
  ...
  ackee.action("505b95be-6ea0-4232-9ca4-2efbb210f9b0", {
    key: "AIS",
    value: 1,
  });
  ...
  function addAIS(data) {
    ...
    if (
      ...
      const m = L.marker(pos, {
        ...
        .bindPopup(
          () =>
            `<b>${name} <span title="${country}">${flag}</span></b><br/><a href="https://www.vesselfinder.com/vessels/details/${mmsi}" target="_blank">${mmsi}</a><br/>COG ${cog}° SOG ${sog}kn<br/><i>${aisStatus[status] ?? ""}</i><br/>age ${formatMMSS(new Date() - time)}`,
        )
        .bindTooltip(() => `${name}`, {
          // permanent: true,
...
export const AISButton = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    var div = L.DomUtil.create(
      "div",
      "nightswitch leaflet-bar leaflet-control",
    );
    ...
    this._layer = L.layerGroup([], {
      attribution:
        '<a href="https://aisstream.io/" target="_blank">aistream</a>',
    });
    this._stop_ais = null;
    map.on("click", () => {});
