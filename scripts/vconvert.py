...
def translate(p, fields, layer):
  ...
  for k1,k2 in fields.items():
    ...
    if v is not None and v not in '#XL':
      ...
      if k2=='HEIGHT' and o[k2]<=0: del o[k2]
      if k2=='ORIENT' and o[k2]<=0: del o[k2]
  o['layer']=layer
  return o
...
def main():
  parser = ArgumentParser(description="converter for vaarwegmarkeringen", formatter_class=ArgumentDefaultsHelpFormatter)
  parser.add_argument('input', help='input file (json)')
  parser.add_argument('output', help='output file (json), overwrite input if omitted',nargs='?')
  ...
  for f in track(load_json(args.input)['features'],'converting'):
    ...
    # print(o)
    if len(o)>1: features.append(feature(o, *ll))
    ...
    o=light(p)
    # print(o)
    if 'COLOUR' in o: features.append(feature(o, *ll))
  ...
  save_json(args.output or args.input, {'type':'FeatureCollection','features':features})
