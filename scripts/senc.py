...
class SENC:
    ...
    def get_type(self):
        ...
        l = self.limit()
        if l:
            self._fd.seek(l)  # skip to next record
        self.limit(-1)
        h = self.unpack("HI")
        if h:
            self.limit(h[1] - 6)
    ...
    def start_record(self, rtype):
        ...
        assert self._stride == 4 or rtype in S57_RECORD_TYPES, (
            f"record type {rtype} {RECORDS[rtype][0]} not allowed in S57"
        )
    ...
    def add_record(self, **data):
        ...
        if "array" in data:
            data["count"] = len(data["array"]) // 2
        if "triangles" in data:
            data["trias"] = len(data["triangles"])
        if "ftype" in data:
            if "id" in data:
                self._fid = data["id"]
            else:
        ...
        if "value" in data:
            v = data["value"]
            data["vtype"] = (
                4 if isinstance(v, str) else 2 if isinstance(v, float) else 0
            )
        self.start_record(t)
        try:
            # print('>',data)
...
def senc2s57(fi, fo, scale=None):
    ...
    with SENC(fo, "w") as s57:
        for r in senc:
            t = r["type"]
            if scale and t == HEADER_CELL_NATIVESCALE:
                r["scale"] = scale
            if t == CELL_TXTDSC_INFO_FILE_RECORD:
                write_txt(join(dirname(fo), r["file"]), r["text"])
            if t in S57_RECORD_TYPES and t not in (
                HEADER_SENC_VERSION,
                ...
                if "edges" in r:  # lines and areas
                    ...
                    for e in r["edges"]:
                        if e[1] and e[3]:  # edge with reverse flag
                            et[eid] = list(
                                reversed(et[e[1]])
                            )  # added flipped add to edge table
                ...
                assert t not in (
                    ...
                    VECTOR_CONNECTED_NODE_TABLE_EXT_RECORD,
                    FEATURE_GEOMETRY_RECORD_AREA_EXT,
                ), t  # stop on EXT record (not implemented)
        s57.add_record(
            type=VECTOR_EDGE_NODE_TABLE_RECORD, edges=et
        )  # write modified edge table
