...
def main():
    ...
    for f in track(features, "[green]filtering[/]"):
        ...
        if cols:
            try:
                props["color"] = "".join(
                    map(abbr_color, map(int, str(cols).split(",")))
                )
            except:
                pass
        ...
        if "catgeo" not in props:
            g = f["geometry"]["type"]
            props["catgeo"] = (
                1 if "Point" in g else 2 if "Line" in g else 3 if "Polygon" in g else 0
            )
        ...
        if args.uband:
            props["uband"] = args.uband
        if args.chart:
            props["chart"] = args.chart
        if args.layer:
            props["layer"] = args.layer
    ...
    if args.bsh:
        features = data["features"] = [
            f
            for f in features
            if re.match(r"DE\d(NO|OS)...", f["properties"].get("chart", "DE2NO000"))
        ]
        assert features
    ...
    for k in sorted(values.keys()):
        ...
        if len(sys.argv) > 3:
            ...
            save_json(
                fn,
                {"type": "FeatureCollection", "features": list(filter(filt, features))},
            )
...
def unique_attributes():
    ...
    unique = {
        a: {g: s.pop() for g, s in d.items() if len(s) == 1}
...
def layer(props):
    ...
    if "BOYSHP" in props and point:
        if "CATLAM" in props or str(props.get("COLOUR", "")) in "34":  # RG
            return "BOYLAT"
        if "CATCAM" in props or all(
            c in str(props.get("COLOUR", "")) for c in "26"
        ):  # BY
            return "BOYCAR"
...
def group_keys(features, log=0, filt=None):
    ...
    if log:
        ...
        for k in sorted(values.keys()):
            ...
            print(
                f"{k:6} ({t.__name__ if t else None}):",
                vals[:200],
                "..." if len(vals) > 200 else "",
            )
